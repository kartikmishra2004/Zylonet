...
const Profile = () => {
  ...
  const { user, token, night } = useAuth();
  ...
  const fetchPosts = async () => {
    const response = await fetch("https://zylonet-server.onrender.com/api/v1/post/readpost", {
      method: "GET",
      headers: {
  ...
  return (
    <div className='md:ml-[15rem] transition-all duration-500 ease-in-out h-screen'>
      <NavShadow />
      <div
        className={`mx-auto w-full pb-2 ${!night ? "bg-[#2a2834]" : "bg-white"} shadow-xl text-gray-900`}>
