...
const socket = io.connect("https://zylonet-server.vercel.app");
                        ...
                        {chat.map((msg, index) => (
                            <div key={index} className={`flex ${msg.senderId === user._id ? "justify-end" : "justify-start"} mb-2 items-start`}>
                                <div className={`flex items-center ${msg.senderId === user._id ? "flex-row-reverse" : ""}`}>
                                    ...
                                    />
                                    <div className={`bg-${msg.senderId === user._id ? "[#00b855]" : "[#bababa]"} ${msg.senderId === user._id ? "text-white" : "text-black"} rounded-lg p-2 shadow max-w-sm`}>
                                        {msg.message}
                                    </div>
                                </div>
                            </div>
